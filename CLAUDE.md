...
`clear_reserve_token_balances (reserve, asset) → balance` reconstructs a reserve's real ERC20 holdings (`balanceOf(reserve)`) purely from event token-flows — distinct from the LP/NAV figures in `clear_reserves`. **Both reserve types are tracked.** Every event that moves an underlying token is applied as a signed delta (`adjustTokenBalance`): `Deposit`/`Withdraw`, `SingleAsset*` (amount ∓ fee), `Rebalanced`/`Swap` (+in/−out), `IOUMinted`/`IOURedeemed` (±amount, 1:1 backing), `FlashLoan` (+fee only — principal round-trips in-tx).

Every event but `Deposit`/`Withdraw` names its token outright (`asset`, `tokenIn`/`tokenOut`, `token`) and is identical across the two reserve types, so it needs no mapping. `Deposit`/`Withdraw` differ, and that is the whole of the difference:

- a **base** reserve carries a positional `amounts[]` aligned with the contract's append-only `assetList`, so `clear_reserve_assets.position` (assigned from the asset count at `AssetAdded` time) maps `amounts[i]` back to its token (`applyReserveAmounts`);
- a **meta** reserve carries named per-leg scalars instead (`baseLpIn`/`nativeIn`, `baseLpOut`/`nativeOut`) and names no address, so the legs are resolved first (`metaLegs` → `applyMetaAmounts`). The **BaseLP leg is the one whose asset IS a base reserve** — a lookup against `clear_reserves.kind = 'base'`, not a convention, so it does not depend on the order `AssetAdded` was emitted in; position order (native first) is only the fallback when the base reserve is not indexed. Either leg resolving empty makes `adjustTokenBalance` a no-op rather than mis-attributing an amount.

Exact only when indexing starts at the reserve's deployment (no missed flow, no missed `AssetAdded`). The plugin does not reconstruct the underlying tokens' own ERC20 `Transfer`s (those aren't reserve events).
...
- **`clear_reserve_value_history`** — **daily** granularity, PK `(reserve, day)` where `day` is the UTC date of the block timestamp: `total_assets` (gross), `iou_debt`, `nav` (= gross − debt, the denominator the contract prices LP shares with) and `total_supply`. All four are read from the `clear_reserve_values` view (below). `snapshotReserveValue` upserts the day's row; the `ON CONFLICT … WHERE existing.block_number <= EXCLUDED.block_number` guard keeps the highest-block snapshot, so each row is that day's **end-of-day** value. It's called at the end of **every** reserve flow handler (deposit/withdraw/single/rebalance/swap/IOU/flash), for both reserve types, *after* that event's balance/supply mutations, so each snapshot is internally consistent. It is deliberately **not** called from `handleTransfer`: every reserve LP mint/burn is paired with a flow event in the same block, and snapshotting at the bare `Transfer` would capture new supply against stale assets.

  A meta reserve's row only moves when the **meta** reserve emits something. Its value depends on the base reserve's NAV, so between two meta events a base-side move leaves the stored figure stale until the meta's next flow event — inherent to event-driven snapshots, not a gap in the deltas.

## Reserve valuation (`clear_reserve_par_values`, `clear_reserve_values`)

Two SQL views in `schema.go`, mirroring each contract's own value views. Two layers, because a meta reserve's BaseLP leg is priced at the **base** reserve's NAV/share and the base therefore has to be valued first. They are `DROP`ped and recreated on every migration, since `CREATE OR REPLACE VIEW` refuses a column change.

- **`clear_reserve_par_values`** values every leg at par (`Σ balance·10^(18−decimals)`) and every IOU supply at par. That IS the definition for a base reserve (`ClearBaseReserve.totalAssets`/`totalIOUDebt`/`totalReserveValue`); for a meta reserve it is only an intermediate.
- **`clear_reserve_values`** is the figure each contract reports. A base reserve passes through; a meta reserve keeps its native leg at par (`ClearMetaReserve.nativeValue`) and converts its BaseLP leg at the base reserve's NAV/share (`baseLpValue` → `IClearBaseReserve.convertToValue`), **including the `+1`/`+1` virtual offset** the contract prices shares with. Its IOU debt is netted the same way (`totalIOUDebt` converts the BaseLP IOU supply too).

Two details are load-bearing: multiply before dividing, and `trunc()` the quotient — that reproduces the contract's integer arithmetic exactly instead of drifting by a rounding step. And the layering is safe from recursion because `ClearMetaReserve.baseReserve` is typed `IClearBaseReserve`: a meta reserve never sits on another meta.
